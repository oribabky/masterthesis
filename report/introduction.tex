...
\section{Delimitations} \label{sec:delimitations}
	...
	\begin{enumerate}
		...
		\item The algorithm belongs to one of the following algorithm families (see \ref{sec:supervised_algorithms}):
			...
			\end{itemize}
	\end{enumerate}
	At least one algorithm per algorithm family, are considered in this project. %The reason for this is that algorithms from same families generally perform well on similar situations. %need cite
...
%feature engineering in data preparation is not considered since the input data received is not expected to be transformed in any way in reality?
