...
\section{Delimitations} \label{sec:delimitations}
	There are many supervised learning algorithms, all of which are not evaluated in detail in this project. %A number of criterias were set on the algorithms to study to ensure that they can be used to solve at least one of the project subtasks. The criterias were set to cover different types of algorithms, as well as to have a balanced amount. 
An algorithm is qualified for evaluation in this project if the following is true:
	\begin{enumerate}
		...
		\item The algorithm is available in Scikit-learn, which is the machine learning software library used in this project. %\cite{WEBSITE:15}
	...
	Some of the sensors, such as the DSC111, are malfunctioning frequently (see the amount of errors reported by DSC111 in figure \ref{img:histogram_surfstatus}). To avoid any complex relationships between functioning and malfunctioning sensors, it was decided to only model non-error behavior by using non-error observations. 
